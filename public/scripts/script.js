...
(function (fn) {
  ...
  $(".owl-carousel").each(function (i, el) {
    ...
    if (data.owlSectionArrows) {
      var $navContainer = $slider
        .closest("section")
        .find(".owl-custom-navigation");
  ...
  $(".stick-menu").each(function (i, el) {
    var checkTimeout,
      ...
      checkFn = function (check, show) {
        ...
        if (shown && scrollPos > unstickPos) {
          $header.height($header.height()).addClass("sticked-menu");
    ...
    $(window).on("scroll", function () {
      ...
      scrollDistance =
        Math[distance > 0 ? "max" : "min"](scrollDistance, 0) + distance;
      scrollPos = newPos;
      if (
        sticked &&
        (scrollPos <= unstickPos || scrollDistance >= hideDistance)
      ) {
        checkFn(false, false);
  ...
  $(".field-file-control").each(function (i, el) {
    ...
    $el
      .on("change.fileField", function () {
        var $wrap = $(this).closest(".field-wrap"),
          $old = $wrap.find(".field-file-old");
        $wrap
          .find(".field-control")
          .val(
            !this.value && $old.length
              ? $old.attr("data-value") || $old.val()
              : this.value
          );
      })
    ...
    $el.on("change.imageField", function () {
      ...
      if (files.length) {
        ...
        $root.removeClass("has-file").find(".file-preview-image").empty();
      }
    });
  ...
  $(".shuffle-js").each(function (i, el) {
    ...
    shuffleInstance.on(Shuffle.EventType.LAYOUT, function () {
      ...
      $el
        .find(".shuffle-empty")
        .css("display", shuffleInstance.visibleItems ? "none" : "block");
    });
  });
  ...
  $("[data-preview-image]").each(function (i, el) {
    ...
    $('[data-preview-image-source="' + name + '"]')
  ...
  initMap = function () {
    ...
    $(".gmap").each(function (i, el) {
      var $el = $(el),
        ...
        center = {
          lat: data.centerLat || mark.lat - 0.002,
          lng: data.centerLng || mark.lng + 0.003,
        },
