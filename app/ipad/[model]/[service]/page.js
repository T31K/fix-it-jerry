...
export async function generateStaticParams() {
  ...
  for (const model of ipadModels) {
    for (const service of ipadServices) {
...
export async function generateMetadata({ params }) {
  ...
  const service = ipadServices.find((s) => s.slug === params.service);
  ...
  return {
    ...
    openGraph: {
      title: service.getTitle(model),
      description: service.getMetaDescription(model),
      images: [`https://www.fixitjerry.com/og_images/ipad-${params.model}-${params.service}.png`],
    },
    alternates: { canonical: `/ipad/${params.model}/${params.service}` },
  };
}

export default function IpadServicePage({ params }) {
  const model = ipadModels.find((m) => m.slug === params.model);
  const service = ipadServices.find((s) => s.slug === params.service);
  ...
  return (
    <div id="wrapper">
        ...
        >
          ...
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href={`/ipad/${model.slug}/${service.slug}`}>{service.name}</a></li>
            </ul>
          </div>
        </section>
              ...
              <div className="col-lg-4">
                <div className="sticky-top" style={{ top: "100px" }}>
                  <div className="bg-white border rounded-xl p-4 shadow-sm">
                    <h5 className="fw-bold mb-3 text-gray-800 text-base">All iPad Models</h5>
      ...
      <ServiceJsonLd
        model={model}
        service={service}
        url={`https://www.fixitjerry.com/ipad/${model.slug}/${service.slug}`}
      />
      <Footer />
      <BreadcrumbJsonLd items={[
        ...
        { name: service.name, url: `/ipad/${model.slug}/${service.slug}` },
      ]} />
    </div>
  );
}
