...
export async function generateStaticParams() {
  return xiaomiModels.map((model) => ({ model: model.slug }));
}

export async function generateMetadata({ params }) {
  const model = xiaomiModels.find((m) => m.slug === params.model);
  ...
  return {
    ...
    openGraph: {
      title: `${model.name} Repair in KL | Fix It Jerry`,
      description: `${model.name} repair in Kuala Lumpur. Screen, battery, charging port, water damage & back glass. Fast turnaround. Walk in or book online.`,
      images: [`https://www.fixitjerry.com/og_images/xiaomi-${params.model}.png`],
    },
    alternates: { canonical: `/xiaomi/${params.model}` },
  };
}

export default function XiaomiModelPage({ params }) {
  const model = xiaomiModels.find((m) => m.slug === params.model);
  if (!model) notFound();
  ...
  return (
    <div id="wrapper">
        ...
        >
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-2xl mx-auto !text-[16px] md:!text-[18px] text-center mt-3">
                  {model.screenSize} · {model.screenType} · {model.chipset} · {model.batteryMah}mAh
                </p>
              </div>
            </div>
          </div>
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href={`/xiaomi/${model.slug}`}>{model.name}</a></li>
            </ul>
          </div>
        </section>
            ...
            <div className="row g-4">
              <div className="col-lg-8">
                ...
                <div className="row g-3">
                  {xiaomiServices.map((service) => (
                    <div key={service.slug} className="col-12 col-md-6">
                      <Link
                        href={`/xiaomi/${model.slug}/${service.slug}`}
                ...
                <div className="mt-8 bg-light rounded-lg p-5">
                  ...
                  <p className="text-gray-700">
                    ...
                    {model.screenType} display at {model.refreshRate}, powered by the{" "}
                    {model.chipset}. It has a {model.batteryMah}mAh battery
                    {model.ipRating && model.ipRating !== "none"
                      ? ` and is rated ${model.ipRating} for water resistance.`
                      : " and has no official IP water-resistance rating."}
                    {model.hasWirelessCharging ? " Supports wireless charging." : ""}
                    {model.isFoldable ? " This is a foldable, so repairs require extra care around the hinge mechanism." : ""}
      ...
      <BreadcrumbJsonLd items={[
        ...
        { name: model.name, url: `/xiaomi/${model.slug}` },
      ]} />
      <Footer />
    </div>
  );
