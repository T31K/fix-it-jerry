...
export async function generateStaticParams() {
  ...
  for (const model of xiaomiModels) {
    for (const service of xiaomiServices) {
...
export async function generateMetadata({ params }) {
  ...
  const service = xiaomiServices.find((s) => s.slug === params.service);
  ...
  return {
    ...
    openGraph: {
      title: service.getTitle(model),
      description: service.getMetaDescription(model),
      images: [`https://www.fixitjerry.com/og_images/xiaomi-${params.model}-${params.service}.png`],
    },
    alternates: { canonical: `/xiaomi/${params.model}/${params.service}` },
  };
}

export default function XiaomiServicePage({ params }) {
  const model = xiaomiModels.find((m) => m.slug === params.model);
  const service = xiaomiServices.find((s) => s.slug === params.service);
  ...
  return (
    <div id="wrapper">
        ...
        >
          ...
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href={`/xiaomi/${model.slug}/${service.slug}`}>{service.name}</a></li>
            </ul>
          </div>
        </section>
            ...
            <div className="row g-4 g-lg-5">
              <div className="col-lg-8">
                <div className="bg-white rounded-xl shadow-sm border p-4 md:p-8">
                    ...
                    <DeviceGallery brand="xiaomi" slug={model.slug} name={model.name} />
              ...
              <div className="col-lg-4">
                <div className="sticky-top" style={{ top: "100px" }}>
                  <div className="bg-white border rounded-xl shadow-sm p-4">
                    <h5 className="fw-bold mb-3 text-gray-800 text-base">All Xiaomi Models</h5>
                      ...
                      {xiaomiModels
                        ...
                        .map((m) => (
                          <li key={m.slug} className="mb-1">
                            <a
                              href={`/xiaomi/${m.slug}/${service.slug}`}
      ...
      <ServiceJsonLd
        model={model}
        service={service}
        url={`https://www.fixitjerry.com/xiaomi/${model.slug}/${service.slug}`}
      />
      <Footer />
      <BreadcrumbJsonLd items={[
        ...
        { name: service.name, url: `/xiaomi/${model.slug}/${service.slug}` },
      ]} />
    </div>
  );
}
