...
export async function generateStaticParams() {
  ...
  for (const model of iphoneModels) {
    for (const service of iphoneServices) {
...
export async function generateMetadata({ params }) {
  ...
  const service = iphoneServices.find((s) => s.slug === params.service);
...
export default function IphoneServicePage({ params }) {
  const model = iphoneModels.find((m) => m.slug === params.model);
  const service = iphoneServices.find((s) => s.slug === params.service);
        ...
        >
          ...
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href={`/iphone/${model.slug}/${service.slug}`}>{service.name}</a></li>
            </ul>
          </div>
        </section>
            ...
            <div className="row g-4 g-lg-5">
              <div className="col-lg-8">
                <div className="bg-white rounded-xl shadow-sm border p-4 md:p-8">
                  ...
                  <DeviceGallery brand="iphone" slug={model.slug} name={model.name} />
