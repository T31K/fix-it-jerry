...
export async function generateStaticParams() {
  const params = [];
  for (const model of pixelModels) {
...
export async function generateMetadata({ params }) {
  const model = pixelModels.find((m) => m.slug === params.model);
  const service = pixelServices.find((s) => s.slug === params.service);
  ...
  return {
    ...
    openGraph: {
      title: service.getTitle(model),
      description: service.getMetaDescription(model),
      images: [`https://www.fixitjerry.com/og_images/pixel-${params.model}-${params.service}.png`],
    },
    alternates: { canonical: `/pixel/${params.model}/${params.service}` },
  };
}

export default function PixelServicePage({ params }) {
  const model = pixelModels.find((m) => m.slug === params.model);
  const service = pixelServices.find((s) => s.slug === params.service);
  ...
  return (
    <div id="wrapper">
        ...
        >
          ...
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href={`/pixel/${model.slug}`}>{model.name}</a></li>
              <li><a href={`/pixel/${model.slug}/${service.slug}`}>{service.name}</a></li>
            </ul>
          </div>
        </section>
            ...
            <div className="row g-4 g-lg-5">
              <div className="col-lg-8">
                <div className="bg-white rounded-xl shadow-sm border p-4 md:p-8">
                    ...
                    <DeviceGallery brand="pixel" slug={model.slug} name={model.name} />
              ...
              <div className="col-lg-4">
                <div className="sticky-top" style={{ top: "100px" }}>
                  <div className="bg-white border rounded-xl shadow-sm p-4">
                    <h5 className="fw-bold mb-3 text-gray-800 text-base">All Pixel Models</h5>
                      ...
                      {pixelModels
                        ...
                        .map((m) => (
                          <li key={m.slug} className="mb-1">
                            <a
                              href={`/pixel/${m.slug}/${service.slug}`}
      ...
      <ServiceJsonLd
        model={model}
        service={service}
        url={`https://www.fixitjerry.com/pixel/${model.slug}/${service.slug}`}
      />
      <Footer />
