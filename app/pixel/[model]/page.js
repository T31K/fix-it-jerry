...
export async function generateStaticParams() {
  return pixelModels.map((model) => ({ model: model.slug }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: `${model.name} Repair in KL | Fix It Jerry`,
      description: `${model.name} repair in Kuala Lumpur. Screen, battery, charging port, water damage & back glass. Fast turnaround. Walk in or book online.`,
      images: [`https://www.fixitjerry.com/og_images/pixel-${params.model}.png`],
    },
    alternates: { canonical: `/pixel/${params.model}` },
  };
}

export default function PixelModelPage({ params }) {
  const model = pixelModels.find((m) => m.slug === params.model);
  if (!model) notFound();
                ...
                <div className="row g-3">
                  {pixelServices.map((service) => (
                    <div key={service.slug} className="col-12 col-md-6">
                      <Link
                        href={`/pixel/${model.slug}/${service.slug}`}
