...
export async function generateStaticParams() {
  ...
  return laptopItem?.services.map((service) => ({
    slug: service.hash,
  })) || [];
}
...
export async function generateMetadata({ params }) {
  const laptopItem = repairItems.find((item) => item.title === "Laptop");
  const service = laptopItem?.services.find((s) => s.hash === params.slug);
  ...
  return {
    ...
    alternates: { canonical: `/devices/laptop/${params.slug}` },
  };
}

export default function LaptopServicePage({ params }) {
  const laptopItem = repairItems.find((item) => item.title === "Laptop");
  const service = laptopItem?.services.find((s) => s.hash === params.slug);
  ...
  return (
    <div id="wrapper">
        ...
        >
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-3xl !text-[16px] md:!text-[20px] text-center mt-3">
                  Professional laptop {service.service.toLowerCase()} service
                </p>
              </div>
            </div>
          </div>
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href={`/devices/laptop/${service.hash}`}>{service.service}</a></li>
            </ul>
          </div>
        </section>
      ...
      <BreadcrumbJsonLd items={[
        ...
        { name: "Laptop", url: "/devices/laptop" },
        { name: service.service, url: `/devices/laptop/${service.hash}` },
      ]} />
    </div>
  );
}
