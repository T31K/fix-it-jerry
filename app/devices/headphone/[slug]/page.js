...
export async function generateStaticParams() {
  const headphoneItem = repairItems.find((item) => item.title === "Headphone");

  return headphoneItem?.services.map((service) => ({
    slug: service.hash,
  })) || [];
}
...
export async function generateMetadata({ params }) {
  const headphoneItem = repairItems.find((item) => item.title === "Headphone");
  const service = headphoneItem?.services.find((s) => s.hash === params.slug);
  ...
  return {
    ...
    alternates: { canonical: `/devices/headphone/${params.slug}` },
  };
}

export default function HeadphoneServicePage({ params }) {
  const headphoneItem = repairItems.find((item) => item.title === "Headphone");
  const service = headphoneItem?.services.find((s) => s.hash === params.slug);
  ...
  return (
    <div id="wrapper">
        ...
        >
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-3xl !text-[16px] md:!text-[20px] text-center mt-3">
                  Professional headphone {service.service.toLowerCase()} service
                </p>
              </div>
            </div>
          </div>
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href="/devices/headphone">Headphone</a></li>
              <li><a href={`/devices/headphone/${service.hash}`}>{service.service}</a></li>
            </ul>
          </div>
        </section>
        ...
        <section className="py-8">
          <div className="container">
            <div className="row g-4">
              <InnerSideBar activeLink="headphone" />
      ...
      <BreadcrumbJsonLd items={[
        ...
        { name: "Headphone", url: "/devices/headphone" },
        { name: service.service, url: `/devices/headphone/${service.hash}` },
      ]} />
    </div>
  );
}
