...
export async function generateStaticParams() {
  const airpodsItem = repairItems.find((item) => item.title === "Airpods");

  return airpodsItem?.services.map((service) => ({
    slug: service.hash,
  })) || [];
}
...
export async function generateMetadata({ params }) {
  const airpodsItem = repairItems.find((item) => item.title === "Airpods");
  const service = airpodsItem?.services.find((s) => s.hash === params.slug);
  ...
  return {
    ...
    alternates: { canonical: `/devices/airpods/${params.slug}` },
  };
}

export default function AirpodsServicePage({ params }) {
  const airpodsItem = repairItems.find((item) => item.title === "Airpods");
  const service = airpodsItem?.services.find((s) => s.hash === params.slug);
  ...
  return (
    <div id="wrapper">
        ...
        >
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-3xl !text-[16px] md:!text-[20px] text-center mt-3">
                  Professional AirPods {service.service.toLowerCase()} service
                </p>
              </div>
            </div>
          </div>
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href="/devices/airpods">AirPods</a></li>
              <li><a href={`/devices/airpods/${service.hash}`}>{service.service}</a></li>
            </ul>
          </div>
        </section>
      ...
      <BreadcrumbJsonLd items={[
        ...
        { name: "AirPods", url: "/devices/airpods" },
        { name: service.service, url: `/devices/airpods/${service.hash}` },
      ]} />
    </div>
  );
}
