...
export default function Page() {
  const airpodItems = repairItems.find((item) => item.title === "Airpods");

  const renderGrid = (type) => {
    const filtered = airpodItems?.services.filter((s) => s.service_type === type) || [];
      ...
      <>
        ...
        <div className="grid grid-cols-2 gap-4 md:hidden">
          {filtered.map((service, idx) => (
            <Link key={idx} href={`/devices/airpods/${service.hash}`}>
          ...
          <div className="container relative z-2">
            <div className="row justify-content-center">
              <div className="col-lg-6 text-center">
                ...
                <h1 className="md:whitespace-nowrap !text-[24px] md:!text-[48px]">AirPods Services</h1>
