...
export async function generateStaticParams() {
  const speakerItem = repairItems.find((item) => item.title === "Speaker");

  return speakerItem?.services.map((service) => ({
    slug: service.hash,
  })) || [];
}
...
export async function generateMetadata({ params }) {
  const speakerItem = repairItems.find((item) => item.title === "Speaker");
  const service = speakerItem?.services.find((s) => s.hash === params.slug);
  ...
  return {
    ...
    alternates: { canonical: `/devices/speaker/${params.slug}` },
  };
}

export default function SpeakerServicePage({ params }) {
  const speakerItem = repairItems.find((item) => item.title === "Speaker");
  const service = speakerItem?.services.find((s) => s.hash === params.slug);
  ...
  return (
    <div id="wrapper">
        ...
        >
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-3xl !text-[16px] md:!text-[20px] text-center mt-3">
                  Professional speaker {service.service.toLowerCase()} service
                </p>
              </div>
            </div>
          </div>
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href="/devices/speaker">Speaker</a></li>
              <li><a href={`/devices/speaker/${service.hash}`}>{service.service}</a></li>
            </ul>
          </div>
        </section>
        ...
        <section className="py-8">
          <div className="container">
            <div className="row g-4">
              <InnerSideBar activeLink="speaker" />
      ...
      <BreadcrumbJsonLd items={[
        ...
        { name: "Speaker", url: "/devices/speaker" },
        { name: service.service, url: `/devices/speaker/${service.hash}` },
      ]} />
    </div>
  );
}
