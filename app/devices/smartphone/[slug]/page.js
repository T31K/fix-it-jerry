...
export async function generateStaticParams() {
  ...
  return smartphoneItem?.services.map((service) => ({
    slug: service.hash,
  })) || [];
}
...
export async function generateMetadata({ params }) {
  const smartphoneItem = repairItems.find((item) => item.title === "Smartphone");
  const service = smartphoneItem?.services.find((s) => s.hash === params.slug);
  ...
  return {
    ...
    alternates: { canonical: `/devices/smartphone/${params.slug}` },
  };
}

export default function SmartphoneServicePage({ params }) {
  const smartphoneItem = repairItems.find((item) => item.title === "Smartphone");
  const service = smartphoneItem?.services.find((s) => s.hash === params.slug);
        ...
        >
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-3xl !text-[16px] md:!text-[20px] text-center mt-3">
                  Professional smartphone {service.service.toLowerCase()} service
                </p>
              </div>
            </div>
          </div>
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href="/devices/smartphone">Smartphone</a></li>
              <li><a href={`/devices/smartphone/${service.hash}`}>{service.service}</a></li>
            </ul>
          </div>
        </section>
