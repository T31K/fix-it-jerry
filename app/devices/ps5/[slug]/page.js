...
export async function generateStaticParams() {
  const ps5Item = repairItems.find((item) => item.title === "PS5");

  return ps5Item?.services.map((service) => ({
    slug: service.hash,
  })) || [];
}
...
export async function generateMetadata({ params }) {
  const ps5Item = repairItems.find((item) => item.title === "PS5");
  const service = ps5Item?.services.find((s) => s.hash === params.slug);
  ...
  return {
    ...
    alternates: { canonical: `/devices/ps5/${params.slug}` },
  };
}

export default function Ps5ServicePage({ params }) {
  const ps5Item = repairItems.find((item) => item.title === "PS5");
  const service = ps5Item?.services.find((s) => s.hash === params.slug);
  ...
  return (
    <div id="wrapper">
        ...
        >
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-3xl !text-[16px] md:!text-[20px] text-center mt-3">
                  Professional PS5 {service.service.toLowerCase()} service
                </p>
              </div>
            </div>
          </div>
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href="/devices/ps5">PS5</a></li>
              <li><a href={`/devices/ps5/${service.hash}`}>{service.service}</a></li>
            </ul>
          </div>
        </section>
      ...
      <BreadcrumbJsonLd items={[
        ...
        { name: "PS5", url: "/devices/ps5" },
        { name: service.service, url: `/devices/ps5/${service.hash}` },
      ]} />
    </div>
  );
}
