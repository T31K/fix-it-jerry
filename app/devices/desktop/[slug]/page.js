...
export async function generateStaticParams() {
  const desktopItem = repairItems.find((item) => item.title === "Desktop");

  return desktopItem?.services.map((service) => ({
    slug: service.hash,
  })) || [];
}
...
export async function generateMetadata({ params }) {
  const desktopItem = repairItems.find((item) => item.title === "Desktop");
  const service = desktopItem?.services.find((s) => s.hash === params.slug);
  ...
  return {
    ...
    alternates: { canonical: `/devices/desktop/${params.slug}` },
  };
}

export default function DesktopServicePage({ params }) {
  const desktopItem = repairItems.find((item) => item.title === "Desktop");
  const service = desktopItem?.services.find((s) => s.hash === params.slug);
  ...
  return (
    <div id="wrapper">
        ...
        >
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-3xl !text-[16px] md:!text-[20px] text-center mt-3">
                  Professional desktop {service.service.toLowerCase()} service
                </p>
              </div>
            </div>
          </div>
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href="/devices/desktop">Desktop</a></li>
              <li><a href={`/devices/desktop/${service.hash}`}>{service.service}</a></li>
            </ul>
          </div>
        </section>
        ...
        <section className="py-8">
          <div className="container">
            <div className="row g-4">
              <InnerSideBar activeLink="desktop" />
      ...
      <BreadcrumbJsonLd items={[
        ...
        { name: "Desktop", url: "/devices/desktop" },
        { name: service.service, url: `/devices/desktop/${service.hash}` },
      ]} />
    </div>
  );
}
