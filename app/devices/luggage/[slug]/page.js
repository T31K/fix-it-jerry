...
export async function generateStaticParams() {
  const luggageItem = repairItems.find((item) => item.title === "Luggage");

  return luggageItem?.services.map((service) => ({
    slug: service.hash,
  })) || [];
}
...
export async function generateMetadata({ params }) {
  const luggageItem = repairItems.find((item) => item.title === "Luggage");
  const service = luggageItem?.services.find((s) => s.hash === params.slug);
  ...
  return {
    ...
    alternates: { canonical: `/devices/luggage/${params.slug}` },
  };
}

export default function LuggageServicePage({ params }) {
  const luggageItem = repairItems.find((item) => item.title === "Luggage");
  const service = luggageItem?.services.find((s) => s.hash === params.slug);
  ...
  return (
    <div id="wrapper">
        ...
        >
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-3xl !text-[16px] md:!text-[20px] text-center mt-3">
                  Professional luggage {service.service.toLowerCase()} service
                </p>
              </div>
            </div>
          </div>
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href="/devices/luggage">Luggage</a></li>
              <li><a href={`/devices/luggage/${service.hash}`}>{service.service}</a></li>
            </ul>
          </div>
        </section>
        ...
        <section className="py-8">
          <div className="container">
            <div className="row g-4">
              <InnerSideBar activeLink="luggage" />
      ...
      <BreadcrumbJsonLd items={[
        ...
        { name: "Luggage", url: "/devices/luggage" },
        { name: service.service, url: `/devices/luggage/${service.hash}` },
      ]} />
    </div>
  );
}
