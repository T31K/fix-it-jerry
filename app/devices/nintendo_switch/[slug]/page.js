...
export async function generateStaticParams() {
  const nintendoSwitchItem = repairItems.find((item) => item.title === "Nintendo Switch");

  return nintendoSwitchItem?.services.map((service) => ({
    slug: service.hash,
  })) || [];
}
...
export async function generateMetadata({ params }) {
  const nintendoSwitchItem = repairItems.find((item) => item.title === "Nintendo Switch");
  const service = nintendoSwitchItem?.services.find((s) => s.hash === params.slug);
...
export default function NintendoSwitchServicePage({ params }) {
  const nintendoSwitchItem = repairItems.find((item) => item.title === "Nintendo Switch");
  const service = nintendoSwitchItem?.services.find((s) => s.hash === params.slug);
  ...
  return (
    <div id="wrapper">
        ...
        >
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-3xl !text-[16px] md:!text-[20px] text-center mt-3">
                  Professional Nintendo Switch {service.service.toLowerCase()} service
                </p>
              </div>
            </div>
          </div>
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href="/devices/nintendo_switch">Nintendo Switch</a></li>
              <li><a href={`/devices/nintendo_switch/${service.hash}`}>{service.service}</a></li>
            </ul>
          </div>
        </section>
        ...
        <section className="py-8">
          <div className="container">
            <div className="row g-4">
              <InnerSideBar activeLink="nintendo_switch" />
      ...
      <BreadcrumbJsonLd items={[
        ...
        { name: service.service, url: `/devices/nintendo_switch/${service.hash}` },
      ]} />
    </div>
  );
}
