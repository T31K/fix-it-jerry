...
export default function Page() {
  ...
  const renderGrid = (type) => {
    const filtered = cameraItem?.services.filter((s) => s.service_type === type) || [];
      ...
      <>
        ...
        <div className="grid grid-cols-2 gap-4 md:hidden">
          {filtered.map((service, idx) => (
            <Link key={idx} href={`/devices/camera/${service.hash}`}>
          ...
          <div className="container relative z-2">
            <div className="row justify-content-center">
              <div className="col-lg-6 text-center">
                ...
                <h1 className="md:whitespace-nowrap !text-[24px] md:!text-[48px]">Camera Services</h1>
                <p className="text-gray-700 max-w-3xl !text-[16px] md:!text-[20px] text-center mt-3">
                  {cameraItem?.description}
                </p>
              </div>
            </div>
          </div>
