...
export async function generateStaticParams() {
  return macbookModels.map((model) => ({ model: model.slug }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description: `${model.name} repair in Kuala Lumpur. Screen, battery, keyboard, charging port & water damage. Fast turnaround. Walk in or book online.`,
    openGraph: {
      title: `${model.name} Repair in KL | Fix It Jerry`,
      description: `${model.name} repair in Kuala Lumpur. Screen, battery, keyboard, charging port & water damage. Fast turnaround. Walk in or book online.`,
      images: [`https://www.fixitjerry.com/og_images/macbook-${params.model}.png`],
    },
    alternates: { canonical: `/macbook/${params.model}` },
  };
}

export default function MacbookModelPage({ params }) {
  const model = macbookModels.find((m) => m.slug === params.model);
  if (!model) notFound();
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-2xl mx-auto !text-[16px] md:!text-[18px] text-center mt-3">
                  {model.screenSize} · {model.screenType} · {model.chipset} · {model.batteryWh}Wh
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="col-lg-8">
                ...
                <div className="row g-3">
                  {macbookServices.map((service) => (
                    <div key={service.slug} className="col-12 col-md-6">
                      <Link
                        href={`/macbook/${model.slug}/${service.slug}`}
                ...
                <div className="mt-8 bg-light rounded-lg p-5">
                  ...
                  <p className="text-gray-700">
                    ...
                    {model.screenType} display, powered by the{" "}
                    {model.chipset}. It has a {model.batteryWh}Wh battery
                    {model.hasMagSafe ? " and charges via MagSafe 3 or USB-C" : " and charges via USB-C"}.
                    {model.hasFan ? "" : " This model has no fan, so it runs completely silent."}
