...
export async function generateStaticParams() {
  ...
  for (const model of samsungModels) {
    for (const service of samsungServices) {
...
export async function generateMetadata({ params }) {
  ...
  const service = samsungServices.find((s) => s.slug === params.service);
  ...
  return {
    ...
    openGraph: {
      title: service.getTitle(model),
      description: service.getMetaDescription(model),
      images: [`https://www.fixitjerry.com/og_images/samsung-${params.model}-${params.service}.png`],
    },
    alternates: { canonical: `/samsung/${params.model}/${params.service}` },
  };
}

export default function SamsungServicePage({ params }) {
  const model = samsungModels.find((m) => m.slug === params.model);
  const service = samsungServices.find((s) => s.slug === params.service);
  ...
  return (
    <div id="wrapper">
        ...
        >
            ...
            <div className="row justify-content-center">
              <div className="col-lg-8 text-center">
                ...
                <p className="text-gray-200 max-w-2xl mx-auto !text-[16px] md:!text-[18px] text-center mt-3">
                  Professional {model.name} {service.name.toLowerCase()} service in Kuala Lumpur
                </p>
              </div>
            </div>
          </div>
          <div className="crumb-wrapper">
            <ul className="crumb">
              ...
              <li><a href={`/samsung/${model.slug}/${service.slug}`}>{service.name}</a></li>
            </ul>
          </div>
        </section>
            ...
            <div className="row g-4 g-lg-5">
              <div className="col-lg-8">
                <div className="bg-white rounded-xl shadow-sm border p-4 md:p-8">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {samsungServices
                          ...
                          .map((s) => (
                            <a
                              key={s.slug}
                              href={`/samsung/${model.slug}/${s.slug}`}
                    ...
                    <DeviceGallery brand="samsung" slug={model.slug} name={model.name} />
              ...
              <div className="col-lg-4">
                <div className="sticky-top" style={{ top: "100px" }}>
                  <div className="bg-white border rounded-xl p-4 shadow-sm">
                    <h5 className="fw-bold mb-3 text-gray-800 text-base">All Samsung Models</h5>
                      ...
                      {samsungModels
                        ...
                        .map((m) => (
                          <li key={m.slug} className="mb-1">
                            <a
                              href={`/samsung/${m.slug}/${service.slug}`}
      ...
      <ServiceJsonLd
        model={model}
        service={service}
        url={`https://www.fixitjerry.com/samsung/${model.slug}/${service.slug}`}
      />
      <Footer />
